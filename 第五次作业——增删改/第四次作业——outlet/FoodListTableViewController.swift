...
class FoodListTableViewController: UITableViewController {
    ...
    func initFoodList() {
        if let localfoodList = loadFoodFile(){
            if localfoodList.count != 0{
                ...
                foodList.append(Food(name: "cake", desc: "sweet",foodAvatar: nil))
                foodList.append(Food(name: "hamburger", desc: "meat",foodAvatar: nil))
            }
        }else{
            print("!!!!!!!!!!!!!!!!!")
        }
    ...
    func saveFoodFile(){
        ...
        //print(NSKeyedUnarchiver.unarchiveObject(withFile: Food.ArchiveURL.path) as? [Food])
    }
...
//         Configure the cell...
        cell.foodAvatarView?.image = foodList[indexPath.row].foodAvatar
        cell.titleLable?.text = foodList[indexPath.row].name
        cell.detailLable?.text = foodList[indexPath.row].desc
